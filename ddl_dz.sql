...
(
	id serial PRIMARY KEY, 			-- первичный ключ
	name varchar(255) NOT NULL, 	-- Название региона
...
(
	id serial PRIMARY KEY,		-- первичный ключ
	name varchar(255) NOT null	-- форма обращения
...
(
	id serial PRIMARY KEY,		-- первичный ключ
	name varchar(255) NOT NULL	-- название языка
) TABLESPACE otus_shop_customers;
...
(
	id serial PRIMARY KEY, 						-- первичный ключ
	name varchar(255) NOT NULL,					-- наименование адреса
...
(
	...
	"level" integer NOT NULL CHECK ("level" >=0) DEFAULT 0, -- уровень узла и показывает количество родителей узла
	left_key integer NOT NULL CHECK ("level" >=0),			-- левый ключ узла и указывает на точку отсчета начала ветки
	right_key integer NOT NULL CHECK ("level" >=0),			-- правый ключ узла и указывает на точку останова конца ветки.
...
(
	id serial PRIMARY KEY, 									-- первичный ключ
	"name" varchar(255) NOT NULL							-- наименование типа
);
